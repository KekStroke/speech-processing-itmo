...
class Wav2Vec2Decoder:
    ...
    def greedy_decode(self, logits: torch.Tensor) -> str:
        ...
        return self._ids_to_text(clean)
    ...
    def beam_search_decode(self, logits: torch.Tensor, return_beams: bool = False):
        ...
        for t in range(T):
            ...
            for prefix, (pb, pnb) in beams.items():
                last_tok = prefix[-1] if prefix else None

                for c in range(V):
                    lp = log_probs[t, c].item()

                    if c == self.blank_token_id:
                        b, nb = new_beams.get(prefix, (NEG_INF, NEG_INF))
                        b = _log_add(b, pb + lp)
                        b = _log_add(b, pnb + lp)
                        new_beams[prefix] = (b, nb)

                    elif c == last_tok:
                        b, nb = new_beams.get(prefix, (NEG_INF, NEG_INF))
                        nb = _log_add(nb, pnb + lp)
                        new_beams[prefix] = (b, nb)

                        ext = prefix + (c,)
                        b, nb = new_beams.get(ext, (NEG_INF, NEG_INF))
                        nb = _log_add(nb, pb + lp)
                        new_beams[ext] = (b, nb)
        ...
        results = []
        for prefix, (pb, pnb) in beams.items():
            results.append((list(prefix), _log_add(pb, pnb)))
    ...
    def beam_search_with_lm(self, logits: torch.Tensor) -> str:
        ...
        def score_beam(prefix, pb, pnb, eos=False):
            acoustic = _log_add(pb, pnb)
            text = self._ids_to_text(list(prefix))
            if not text:
                return acoustic
            lm_score = self.lm_model.score(text, bos=True, eos=eos) * math.log(10)
            word_count = len(text.split())
            return acoustic + self.alpha * lm_score + self.beta * word_count
        ...
        for t in range(T):
            ...
            top = heapq.nlargest(
                self.beam_width,
                new_beams.items(),
                key=lambda x: score_beam(x[0], x[1][0], x[1][1]),
            )
            beams = dict(top)

        best_prefix = max(
            beams,
            key=lambda pf: score_beam(pf, beams[pf][0], beams[pf][1], eos=True),
        )
    ...
    def lm_rescore(self, beams: List[Tuple[List[int], float]]) -> str:
        ...
        for token_ids, acoustic_score in beams:
            ...
            if text:
                ...
                word_count = len(text.split())
                score = acoustic_score + self.alpha * lm_score + self.beta * word_count
            else:
